...
import System.Directory (doesFileExist, renameFile, removeFile, getDirectoryContents, removeDirectoryRecursive, createDirectoryIfMissing) 
...
redo target = do
  ...
  where redo' :: FilePath -> IO ()
        redo' path = do
          ...
          Prelude.writeFile (metaDepsDir </> path) =<< md5' path 
...
upToDate target metaDepsDir = catch
  (do deps <- getDirectoryContents metaDepsDir
      (traceShow' . Prelude.all id) `liftM` mapM depUpToDate deps)
  (\(e :: IOException) -> return False)
  where depUpToDate dep = catch
